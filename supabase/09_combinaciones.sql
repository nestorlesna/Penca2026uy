-- ============================================================
-- 09_COMBINACIONES.SQL — Penca Mundial 2026
-- Tabla FIFA de 495 combinaciones de mejores terceros.
-- Define qué grupo enfrenta a cada primero según los 8
-- grupos cuyos terceros clasificaron a 16avos.
-- También reescribe populate_knockout_matches() para usar
-- esta tabla en lugar del enfoque "mejor del pool".
-- Ejecutar DESPUÉS de 08_group_overrides.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS combinaciones (
  id          INT        PRIMARY KEY,
  combinacion VARCHAR(8) NOT NULL UNIQUE,  -- 8 letras de grupos ordenadas
  rival_1a    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo A
  rival_1b    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo B
  rival_1d    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo D
  rival_1e    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo E
  rival_1g    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo G
  rival_1i    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo I
  rival_1k    VARCHAR(3) NOT NULL,  -- 3° que enfrenta al 1° del grupo K
  rival_1l    VARCHAR(3) NOT NULL   -- 3° que enfrenta al 1° del grupo L
);
...
GRANT SELECT              ON public.combinaciones TO anon, authenticated;
GRANT UPDATE              ON public.combinaciones TO authenticated;

CREATE POLICY "comb_public_read"  ON combinaciones FOR SELECT USING (true);
CREATE POLICY "comb_admin_write"  ON combinaciones FOR ALL
  USING ((SELECT is_admin FROM profiles WHERE id = auth.uid()));

-- ── Datos: 495 combinaciones FIFA (C(12,8) = 495) ────────────────────────────

INSERT INTO combinaciones (id, combinacion, rival_1a, rival_1b, rival_1d, rival_1e, rival_1g, rival_1i, rival_1k, rival_1l)
VALUES
...
-- ============================================================
-- REESCRITURA DE populate_knockout_matches()
-- Usa la tabla combinaciones para asignar terceros a 16avos.
-- Reemplaza el enfoque "mejor del pool" que era incorrecto.
-- ============================================================
...
BEGIN
  ...
  FOR v_rule IN SELECT * FROM knockout_slot_rules ORDER BY match_id, slot LOOP
    ...
    ELSIF v_rule.rule_type = 'best_third' THEN
      -- Solo proceder cuando tenemos los 8 terceros clasificados
      IF v_comb_key IS NOT NULL AND LENGTH(v_comb_key) = 8 THEN
        -- Buscar el grupo del local de este partido via knockout_slot_rules
        -- (más robusto que leer home_team_id que puede no estar seteado aún)
        ...
        -- Obtener el código del tercer rival (ej. '3E') según la tabla FIFA
        ...
        -- Extraer letra del grupo del código ('3E' → 'E') y buscar el equipo
